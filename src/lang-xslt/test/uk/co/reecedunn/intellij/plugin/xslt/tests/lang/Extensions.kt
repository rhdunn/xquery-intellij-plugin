...
fun LanguageTestCase.parseXml(resource: String): XmlFile {
...
fun <T> LanguageTestCase.parse(resource: String, namespaceUri: String, localName: String): List<T> {
...
fun LanguageTestCase.element(resource: String, localName: String): List<XmlTag> {
    return parseXml(resource).walkTree().filterIsInstance<XmlTag>().filter { e ->
        e.namespace == XSLT.NAMESPACE && e.localName == localName
    }.toList()
}

fun LanguageTestCase.attribute(resource: String, elementName: String, attributeName: String): List<XmlAttributeValue> {
