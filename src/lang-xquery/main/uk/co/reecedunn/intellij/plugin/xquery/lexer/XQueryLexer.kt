...
class XQueryLexer : XPathLexer() {
    ...
    override fun advance(state: Int): Unit = when (state) {
        ...
        STATE_DEFAULT_ATTRIBUTE_QUOT -> mType = stateDefault(state)
        STATE_DEFAULT_ATTRIBUTE_APOSTROPHE -> mType = stateDefault(state)
        STATE_DEFAULT_ELEM_CONTENT -> mType = stateDefault(state)
        STATE_DEFAULT_STRING_INTERPOLATION -> mType = stateDefault(state)
        STATE_MAYBE_DIR_ELEM_CONSTRUCTOR -> mType = stateDefault(state)
