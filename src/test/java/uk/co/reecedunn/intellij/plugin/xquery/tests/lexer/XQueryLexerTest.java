...
public class XQueryLexerTest extends TestCase {
    ...
    public void testXQDocComment() {
        ...
        matchToken(lexer, "Test :", 29,  4, 10, XQueryTokenType.COMMENT);
        matchToken(lexer, "",        6, 10, 10, XQueryTokenType.UNEXPECTED_END_OF_BLOCK);
        matchToken(lexer, "",        0, 10, 10, null);
    ...
    public void testXQDoc_Trim() {
        ...
        lexer.start("(:~ \t\n \tMultiline \t\n \tComment \t\n \t:)");
        matchToken(lexer, "(:~",           0,  0,  3, XQueryTokenType.XQDOC_START_TAG);
        matchToken(lexer, " \t",          29,  3,  5, XQueryTokenType.WHITE_SPACE);
        ...
        matchToken(lexer, " \t",          29,  9, 11, XQueryTokenType.WHITE_SPACE);
        matchToken(lexer, "Multiline \t", 29, 11, 22, XQueryTokenType.COMMENT);
        matchToken(lexer, "\n \t:",       29, 22, 26, XQueryTokenType.XQDOC_TRIM);
        matchToken(lexer, " \t",          29, 26, 28, XQueryTokenType.WHITE_SPACE);
        matchToken(lexer, "Comment \t",   29, 28, 37, XQueryTokenType.COMMENT);
    ...
    public void testXQDoc_TaggedContents() {
        ...
        matchToken(lexer, "",      33,  9,  9, XQueryTokenType.XQDOC_TAG_CONTENTS_START);
        matchToken(lexer, ":)",    31,  9, 11, XQueryTokenType.COMMENT_END_TAG);
        matchToken(lexer, "",       0, 11, 11, null);
        ...
        matchToken(lexer, "(:~",    0,  0,  3, XQueryTokenType.XQDOC_START_TAG);
        matchToken(lexer, "@",     29,  3,  4, XQueryTokenType.XQDOC_TAG_INDICATOR);
        matchToken(lexer, "",      30,  4,  4, XQueryTokenType.XQDOC_TAG_CONTENTS_START);
        matchToken(lexer, "!£$%^", 31,  4,  9, XQueryTokenType.COMMENT);
        matchToken(lexer, "",      32,  9,  9, XQueryTokenType.XQDOC_TRIM);
        matchToken(lexer, ":)",    31,  9, 11, XQueryTokenType.COMMENT_END_TAG);
        matchToken(lexer, "",       0, 11, 11, null);
        ...
        matchToken(lexer, "(:~",           0,  0,  3, XQueryTokenType.XQDOC_START_TAG);
        matchToken(lexer, "@",            29,  3,  4, XQueryTokenType.XQDOC_TAG_INDICATOR);
        matchToken(lexer, "abc",          30,  4,  7, XQueryTokenType.XQDOC_TAG_NAME);
        matchToken(lexer, "",             33,  7,  7, XQueryTokenType.XQDOC_TAG_CONTENTS_START);
        matchToken(lexer, " one two ",    31,  7, 16, XQueryTokenType.COMMENT);
        matchToken(lexer, "\n :",         29, 16, 19, XQueryTokenType.XQDOC_TRIM);
        ...
        matchToken(lexer, "",             33, 23, 23, XQueryTokenType.XQDOC_TAG_CONTENTS_START);
    ...
    @Specification(name="xqDoc", reference="https://raw.githubusercontent.com/xquery/xquerydoc/master/ebnf/XQDocComments.ebnf")
    public void testXQDoc_TaggedContents_TagIndicatorInMiddleOfContents() {
        Lexer lexer = new XQueryLexer();

        // NOTE: According to the xqDoc grammar, this should be tokenized with
        // @two and @four being TaggedContent. However, this is used in the
        // https://github.com/LeoWoerteler/xq-modules project to indicate email
        // addresses, and the generated xqdoc extracts this correctly.
    ...
    @Specification(name="xqDoc", reference="https://raw.githubusercontent.com/xquery/xquerydoc/master/ebnf/XQDocComments.ebnf")
    public void testXQDoc_TaggedContents_SpaceBeforeTagIndicator() {
        Lexer lexer = new XQueryLexer();

        // NOTE: According to the xqDoc grammar, this should be tokenized as
        // a Contents token containing the whitespace, and a TaggedContents
        // construct with the tag. However, this interferes with the logic
        // to not tokenize '@' characters in the middle of a Contents token.
        //
        // This is needed for the documentation in the
        // https://github.com/LeoWoerteler/xq-modules project to recognise
        ...
        matchToken(lexer, "",             33,  8,  8, XQueryTokenType.XQDOC_TAG_CONTENTS_START);
        matchToken(lexer, " one two ",    31,  8, 17, XQueryTokenType.COMMENT);
        matchToken(lexer, "\n :",         29, 17, 20, XQueryTokenType.XQDOC_TRIM);
        matchToken(lexer, " ",            29, 20, 21, XQueryTokenType.WHITE_SPACE);
        ...
        matchToken(lexer, "",             33, 25, 25, XQueryTokenType.XQDOC_TAG_CONTENTS_START);
    ...
    @Specification(name="xqDoc", reference="https://raw.githubusercontent.com/xquery/xquerydoc/master/ebnf/XQDocComments.ebnf")
    public void testXQDoc_DirElemConstructor() {
        Lexer lexer = new XQueryLexer();

        lexer.start("(:~ @abc one <two /> three :)");
        matchToken(lexer, "(:~",      0,  0,  3, XQueryTokenType.XQDOC_START_TAG);
        matchToken(lexer, " ",       29,  3,  4, XQueryTokenType.WHITE_SPACE);
        matchToken(lexer, "@",       29,  4,  5, XQueryTokenType.XQDOC_TAG_INDICATOR);
        matchToken(lexer, "abc",     30,  5,  8, XQueryTokenType.XQDOC_TAG_NAME);
        matchToken(lexer, "",        33,  8,  8, XQueryTokenType.XQDOC_TAG_CONTENTS_START);
        matchToken(lexer, " one ",   31,  8, 13, XQueryTokenType.COMMENT);
        matchToken(lexer, "<",       31, 13, 14, XQueryTokenType.XQDOC_OPEN_XML_TAG);
        matchToken(lexer, "two",     34, 14, 17, XQueryTokenType.XQDOC_XML_TAG_NAME);
        matchToken(lexer, " ",       34, 17, 18, XQueryTokenType.WHITE_SPACE);
        matchToken(lexer, "/>",      34, 18, 20, XQueryTokenType.XQDOC_SELF_CLOSING_XML_TAG);
        matchToken(lexer, " three ", 31, 20, 27, XQueryTokenType.COMMENT);
        matchToken(lexer, "",        32, 27, 27, XQueryTokenType.XQDOC_TRIM);
        matchToken(lexer, ":)",      31, 27, 29, XQueryTokenType.COMMENT_END_TAG);
        matchToken(lexer, "",         0, 29, 29, null);

        lexer.start("(:~ @abc one <two >three</two > four :)");
        matchToken(lexer, "(:~",     0,  0,  3, XQueryTokenType.XQDOC_START_TAG);
        matchToken(lexer, " ",      29,  3,  4, XQueryTokenType.WHITE_SPACE);
        matchToken(lexer, "@",      29,  4,  5, XQueryTokenType.XQDOC_TAG_INDICATOR);
        matchToken(lexer, "abc",    30,  5,  8, XQueryTokenType.XQDOC_TAG_NAME);
        ...
        matchToken(lexer, "two",    34, 14, 17, XQueryTokenType.XQDOC_XML_TAG_NAME);
        matchToken(lexer, " ",      34, 17, 18, XQueryTokenType.WHITE_SPACE);
        matchToken(lexer, ">",      34, 18, 19, XQueryTokenType.XQDOC_END_XML_TAG);
        matchToken(lexer, "three",  37, 19, 24, XQueryTokenType.XQDOC_XML_ELEM_CONTENTS);
        matchToken(lexer, "</",     37, 24, 26, XQueryTokenType.XQDOC_CLOSE_XML_TAG);
        matchToken(lexer, "two",    38, 26, 29, XQueryTokenType.XQDOC_XML_TAG_NAME);
        matchToken(lexer, " ",      38, 29, 30, XQueryTokenType.WHITE_SPACE);
        matchToken(lexer, ">",      38, 30, 31, XQueryTokenType.XQDOC_END_XML_TAG);
        matchToken(lexer, " four ", 31, 31, 37, XQueryTokenType.COMMENT);
        matchToken(lexer, "",       32, 37, 37, XQueryTokenType.XQDOC_TRIM);
        matchToken(lexer, ":)",     31, 37, 39, XQueryTokenType.COMMENT_END_TAG);
        matchToken(lexer, "",        0, 39, 39, null);

        lexer.start("(:~ @abc one <two-three /> four :)");
        matchToken(lexer, "(:~",     0,  0,  3, XQueryTokenType.XQDOC_START_TAG);
        matchToken(lexer, " ",      29,  3,  4, XQueryTokenType.WHITE_SPACE);
        matchToken(lexer, "@",      29,  4,  5, XQueryTokenType.XQDOC_TAG_INDICATOR);
        matchToken(lexer, "abc",    30,  5,  8, XQueryTokenType.XQDOC_TAG_NAME);
        ...
        matchToken(lexer, "<",      31, 13, 14, XQueryTokenType.XQDOC_OPEN_XML_TAG);
        matchToken(lexer, "two",    34, 14, 17, XQueryTokenType.XQDOC_XML_TAG_NAME);
        matchToken(lexer, "-",      34, 17, 18, XQueryTokenType.INVALID);
        matchToken(lexer, "three",  34, 18, 23, XQueryTokenType.XQDOC_XML_TAG_NAME);
        matchToken(lexer, " ",      34, 23, 24, XQueryTokenType.WHITE_SPACE);
        matchToken(lexer, "/>",     34, 24, 26, XQueryTokenType.XQDOC_SELF_CLOSING_XML_TAG);
        matchToken(lexer, " four ", 31, 26, 32, XQueryTokenType.COMMENT);
        matchToken(lexer, "",       32, 32, 32, XQueryTokenType.XQDOC_TRIM);
        matchToken(lexer, ":)",     31, 32, 34, XQueryTokenType.COMMENT_END_TAG);
    ...
    @Specification(name="xqDoc", reference="https://raw.githubusercontent.com/xquery/xquerydoc/master/ebnf/XQDocComments.ebnf")
    public void testXQDoc_DirAttrConstructor() {
        Lexer lexer = new XQueryLexer();

        lexer.start("(:~ @abc one <two abc=\"def\" ghi=\"jkl\">three</two > four :)");
        matchToken(lexer, "(:~",     0,  0,  3, XQueryTokenType.XQDOC_START_TAG);
        matchToken(lexer, " ",      29,  3,  4, XQueryTokenType.WHITE_SPACE);
        matchToken(lexer, "@",      29,  4,  5, XQueryTokenType.XQDOC_TAG_INDICATOR);
        matchToken(lexer, "abc",    30,  5,  8, XQueryTokenType.XQDOC_TAG_NAME);
        ...
        matchToken(lexer, "two",    34, 14, 17, XQueryTokenType.XQDOC_XML_TAG_NAME);
        matchToken(lexer, " ",      34, 17, 18, XQueryTokenType.WHITE_SPACE);
        matchToken(lexer, "abc",    34, 18, 21, XQueryTokenType.XQDOC_XML_TAG_NAME);
        matchToken(lexer, "=",      34, 21, 22, XQueryTokenType.XQDOC_XML_EQUAL);
        matchToken(lexer, "\"",     34, 22, 23, XQueryTokenType.XQDOC_XML_ATTRIBUTE_VALUE_START);
        matchToken(lexer, "def",    35, 23, 26, XQueryTokenType.XQDOC_XML_ATTRIBUTE_VALUE_CONTENTS);
        matchToken(lexer, "\"",     35, 26, 27, XQueryTokenType.XQDOC_XML_ATTRIBUTE_VALUE_END);
        matchToken(lexer, " ",      34, 27, 28, XQueryTokenType.WHITE_SPACE);
        matchToken(lexer, "ghi",    34, 28, 31, XQueryTokenType.XQDOC_XML_TAG_NAME);
        matchToken(lexer, "=",      34, 31, 32, XQueryTokenType.XQDOC_XML_EQUAL);
        matchToken(lexer, "\"",     34, 32, 33, XQueryTokenType.XQDOC_XML_ATTRIBUTE_VALUE_START);
        matchToken(lexer, "jkl",    35, 33, 36, XQueryTokenType.XQDOC_XML_ATTRIBUTE_VALUE_CONTENTS);
        matchToken(lexer, "\"",     35, 36, 37, XQueryTokenType.XQDOC_XML_ATTRIBUTE_VALUE_END);
        matchToken(lexer, ">",      34, 37, 38, XQueryTokenType.XQDOC_END_XML_TAG);
        matchToken(lexer, "three",  37, 38, 43, XQueryTokenType.XQDOC_XML_ELEM_CONTENTS);
        matchToken(lexer, "</",     37, 43, 45, XQueryTokenType.XQDOC_CLOSE_XML_TAG);
        matchToken(lexer, "two",    38, 45, 48, XQueryTokenType.XQDOC_XML_TAG_NAME);
        matchToken(lexer, " ",      38, 48, 49, XQueryTokenType.WHITE_SPACE);
        matchToken(lexer, ">",      38, 49, 50, XQueryTokenType.XQDOC_END_XML_TAG);
        matchToken(lexer, " four ", 31, 50, 56, XQueryTokenType.COMMENT);
        matchToken(lexer, "",       32, 56, 56, XQueryTokenType.XQDOC_TRIM);
        matchToken(lexer, ":)",     31, 56, 58, XQueryTokenType.COMMENT_END_TAG);
        matchToken(lexer, "",        0, 58, 58, null);
        ...
        matchToken(lexer, "=",      34, 21, 22, XQueryTokenType.XQDOC_XML_EQUAL);
        matchToken(lexer, "'",      34, 22, 23, XQueryTokenType.XQDOC_XML_ATTRIBUTE_VALUE_START);
        matchToken(lexer, "def",    36, 23, 26, XQueryTokenType.XQDOC_XML_ATTRIBUTE_VALUE_CONTENTS);
        matchToken(lexer, "'",      36, 26, 27, XQueryTokenType.XQDOC_XML_ATTRIBUTE_VALUE_END);
        matchToken(lexer, " ",      34, 27, 28, XQueryTokenType.WHITE_SPACE);
        matchToken(lexer, "ghi",    34, 28, 31, XQueryTokenType.XQDOC_XML_TAG_NAME);
        matchToken(lexer, "=",      34, 31, 32, XQueryTokenType.XQDOC_XML_EQUAL);
        matchToken(lexer, "'",      34, 32, 33, XQueryTokenType.XQDOC_XML_ATTRIBUTE_VALUE_START);
        matchToken(lexer, "jkl",    36, 33, 36, XQueryTokenType.XQDOC_XML_ATTRIBUTE_VALUE_CONTENTS);
        matchToken(lexer, "'",      36, 36, 37, XQueryTokenType.XQDOC_XML_ATTRIBUTE_VALUE_END);
        matchToken(lexer, ">",      34, 37, 38, XQueryTokenType.XQDOC_END_XML_TAG);
        matchToken(lexer, "three",  37, 38, 43, XQueryTokenType.XQDOC_XML_ELEM_CONTENTS);
        matchToken(lexer, "</",     37, 43, 45, XQueryTokenType.XQDOC_CLOSE_XML_TAG);
        matchToken(lexer, "two",    38, 45, 48, XQueryTokenType.XQDOC_XML_TAG_NAME);
