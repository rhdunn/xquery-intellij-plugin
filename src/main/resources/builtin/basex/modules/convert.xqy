...
declare %a:since("basex", "7.3") %a:until("basex", "9.0") %a:see-also("basex", "9.0", "convert:integers-to-base64") function convert:bytes-to-base64($input as xs:byte*) as xs:base64Binary (: $input [7.3]xs:byte [8.7]xs:integer :) external;
declare %a:since("basex", "9.0") function convert:integers-to-base64($integers as xs:integer*) as xs:base64Binary external;
declare %a:since("basex", "7.3") %a:until("basex", "9.0") %a:see-also("basex", "9.0", "convert:integers-to-hex") function convert:bytes-to-hex($input as xs:integer*) as xs:hexBinary (: $input [7.3]xs:byte [8.7]xs:integer :) external;
