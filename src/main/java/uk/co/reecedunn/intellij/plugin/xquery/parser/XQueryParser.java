...
class XQueryParser extends PsiTreeParser {
    ...
    private ParseStatus parseBlockVarDecl() {
        final PsiBuilder.Marker blockVarDeclMarker = matchTokenTypeWithMarker(XQueryTokenType.K_DECLARE);
    ...
    private boolean parseForOrWindowClause() {
        final PsiBuilder.Marker forClauseMarker = matchTokenTypeWithMarker(XQueryTokenType.K_FOR);
    ...
    private boolean parseTumblingWindowClause() {
        final PsiBuilder.Marker tumblingWindowClauseMarker = matchTokenTypeWithMarker(XQueryTokenType.K_TUMBLING);
            ...
            if (parseTypeDeclaration()) {
                ...
                if (!parseExprSingle() && !haveErrors) {
                    ...
                    haveErrors = true;
                      ...
                      (haveErrors = errorOnTokenType(XQueryTokenType.EQUAL, XQueryBundle.message("parser.error.expected", ":=")))) {
                parseWhiteSpaceAndCommentTokens();
                if (!parseExprSingle() && !haveErrors) {
                    error(XQueryBundle.message("parser.error.expected-expression"));
                    haveErrors = true;
    ...
    private boolean parseValidateExpr() {
        ...
        if (validateExprMarker != null) {
            parseWhiteSpaceAndCommentTokens();
            BlockOpen blockOpen = BlockOpen.REQUIRED;
    ...
    private boolean parseFTExtensionSelection() {
        final PsiBuilder.Marker extensionSelectionMarker = mark();
        boolean haveError = false;

        boolean havePragma = false;
        while (parsePragma()) {
            parseWhiteSpaceAndCommentTokens();
            havePragma = true;
        }
    ...
    private ParseStatus parseElementDeclTest() {
        final PsiBuilder.Marker elementDeclTestMarker = matchTokenTypeWithMarker(XQueryTokenType.K_ELEMENT_DECL);
    ...
    private ParseStatus parseSchemaParticleTest() {
        final PsiBuilder.Marker schemaParticleTestMarker = matchTokenTypeWithMarker(XQueryTokenType.K_SCHEMA_PARTICLE);
    ...
    private ParseStatus parseSchemaRootTest() {
        final PsiBuilder.Marker schemaRootTestMarker = matchTokenTypeWithMarker(XQueryTokenType.K_SCHEMA_ROOT);
    ...
    private ParseStatus parseSchemaTypeTest() {
        final PsiBuilder.Marker schemaTypeTestMarker = matchTokenTypeWithMarker(XQueryTokenType.K_SCHEMA_TYPE);
    ...
    private ParseStatus parseSimpleTypeTest() {
        final PsiBuilder.Marker simpleTypeTestMarker = matchTokenTypeWithMarker(XQueryTokenType.K_SIMPLE_TYPE);
