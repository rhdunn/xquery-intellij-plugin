...
class XQueryParser : XPathParser() {
    ...
    override val EXPR: IElementType = XQueryElementType2.EXPR
    ...
    private fun parseModule(builder: PsiBuilder, isFirst: Boolean): Boolean {
        ...
        if (isFirst) {
            builder.error(XQueryPluginBundle.message("parser.error.expected-module-type"))
        }
        marker.drop()
        ...
        if (marker != null) {
            ...
            if (builder.matchTokenType(XQueryTokenType.K_ENCODING)) {
                ...
                if (!parseStringLiteral(builder)) {
                    builder.error(XQueryPluginBundle.message("parser.error.expected-encoding-string"))
                    haveErrors = true
                }
                ...
                if (!parseStringLiteral(builder) && !haveErrors) {
                    builder.error(XQueryPluginBundle.message("parser.error.expected-version-string"))
                    haveErrors = true
                }
    ...
    private fun parseModuleDecl(builder: PsiBuilder): Boolean {
        ...
        if (marker != null) {
            ...
            parseWhiteSpaceAndCommentTokens(builder)
            if (!parseQNameOrWildcard(builder, XQueryElementType2.NCNAME, false) && !haveErrors) {
                builder.error(XQueryPluginBundle.message("parser.error.expected-ncname"))
                haveErrors = true
            }
    ...
    private fun parseDefaultDecl(builder: PsiBuilder, marker: PsiBuilder.Marker, state: PrologDeclState): Boolean {
        ...
        if (errorMarker != null) {
            if (state == PrologDeclState.HEADER_STATEMENT) {
                ...
                errorMarker.error(XQueryPluginBundle.message("parser.error.expected-prolog-body"))
            }
    ...
    private fun parseNamespaceDecl(builder: PsiBuilder, state: PrologDeclState): Boolean {
        ...
        if (errorMarker != null) {
            ...
            parseWhiteSpaceAndCommentTokens(builder)
            if (!parseQNameOrWildcard(builder, XQueryElementType2.NCNAME, false)) {
                builder.error(XQueryPluginBundle.message("parser.error.expected-ncname"))
                haveErrors = true
            }
    ...
    private fun parseDecimalFormatDecl(builder: PsiBuilder, state: PrologDeclState, isDefault: Boolean): Boolean {
        ...
        if (errorMarker != null) {
            ...
            if (!isDefault) {
                parseWhiteSpaceAndCommentTokens(builder)
                if (!parseEQNameOrWildcard(builder, XQueryElementType2.QNAME, false)) {
            ...
            while (parseDFPropertyName(builder)) {
                ...
                parseWhiteSpaceAndCommentTokens(builder)
                if (!parseStringLiteral(builder) && !haveErrors) {
                    builder.error(XQueryPluginBundle.message("parser.error.expected-property-value-string"))
                    haveErrors = true
                }
    ...
    private fun parseContextItemDecl(builder: PsiBuilder): Boolean {
        if (builder.matchTokenType(XQueryTokenType.K_CONTEXT)) {
            ...
            parseWhiteSpaceAndCommentTokens(builder)
            if (builder.matchTokenType(XPathTokenType.ASSIGN_EQUAL) || { haveErrors = builder.errorOnTokenType(XPathTokenType.EQUAL, XQueryPluginBundle.message("parser.error.expected-variable-value")); haveErrors }()) {
                parseWhiteSpaceAndCommentTokens(builder)
                ...
                builder.error(XQueryPluginBundle.message("parser.error.expected-variable-value"))
    ...
    private fun parseVarDecl(builder: PsiBuilder): Boolean {
        if (builder.matchTokenType(XQueryTokenType.K_VARIABLE)) {
            ...
            parseWhiteSpaceAndCommentTokens(builder)
            if (builder.matchTokenType(XPathTokenType.ASSIGN_EQUAL) || { haveErrors = haveErrors or builder.errorOnTokenType(XPathTokenType.EQUAL, XQueryPluginBundle.message("parser.error.expected-variable-value")); haveErrors }()) {
                parseWhiteSpaceAndCommentTokens(builder)
                if (!parseExprSingle(builder, XQueryElementType.VAR_VALUE) && !haveErrors) {
                    builder.error(XPathBundle.message("parser.error.expected-expression"))
                }
    ...
    private fun parseFunctionDecl(builder: PsiBuilder, marker: PsiBuilder.Marker, firstAnnotation: IElementType?): Boolean {
        if (builder.matchTokenType(XPathTokenType.K_FUNCTION)) {
            ...
            if (
                ...
                builder.error(XQueryPluginBundle.message("parser.error.expected-enclosed-expression-or-keyword", "external"))
                parseExpr(builder, XQueryElementType2.EXPR, true)
    ...
    private fun parseOptionDecl(builder: PsiBuilder): Boolean {
        if (builder.matchTokenType(XQueryTokenType.K_OPTION)) {
            ...
            parseWhiteSpaceAndCommentTokens(builder)
            if (!parseStringLiteral(builder) && !haveErrors) {
                builder.error(XQueryPluginBundle.message("parser.error.expected-option-string"))
            }
    ...
    private fun parseTypeDecl(builder: PsiBuilder): Boolean {
        if (builder.matchTokenType(XQueryTokenType.K_TYPE)) {
            ...
            parseWhiteSpaceAndCommentTokens(builder)
            if (!parseQNameOrWildcard(builder, XQueryElementType2.QNAME, false)) {
                builder.error(XQueryPluginBundle.message("parser.error.expected-qname"))
                haveErrors = true
            }
    ...
    override fun parseEnclosedExprOrBlock(
        ...
        var exprType = XQueryElementType2.EXPR
    ...
    private fun parseBlockVarDeclEntry(builder: PsiBuilder): ParseStatus {
        ...
        parseWhiteSpaceAndCommentTokens(builder)
        if (builder.matchTokenType(XPathTokenType.ASSIGN_EQUAL) || { haveErrors = builder.errorOnTokenType(XPathTokenType.EQUAL, XQueryPluginBundle.message(errorMessage)); haveErrors }()) {
            parseWhiteSpaceAndCommentTokens(builder)
            ...
            builder.error(XQueryPluginBundle.message(errorMessage))
    ...
    private fun parseApplyExpr(builder: PsiBuilder, type: IElementType, functionDeclRecovery: Boolean): Boolean {
        ...
        while (true) {
            if (!parseConcatExpr(builder)) {
                parseWhiteSpaceAndCommentTokens(builder)
                if (functionDeclRecovery || !builder.errorOnTokenType(XQueryTokenType.SEPARATOR, XQueryPluginBundle.message("parser.error.expected-query-statement", ";"))) {
    ...
    private fun parseFLWORExpr(builder: PsiBuilder): Boolean {
        ...
        if (parseInitialClause(builder)) {
            ...
            return true
        } else if (builder.errorOnTokenType(XPathTokenType.K_RETURN, XQueryPluginBundle.message("parser.error.return-without-flwor"))) {
            parseWhiteSpaceAndCommentTokens(builder)
    ...
    override fun parseLetBinding(builder: PsiBuilder, isFirst: Boolean): Boolean {
        ...
        if (!matched) {
            builder.error(XQueryPluginBundle.message("parser.error.expected-keyword-or-token", "$", "score"))
            haveErrors = true
        }
        ...
        if (matched || !isFirst) {
            ...
            if (haveVariableIndicator) {
                ...
                errorMessage = if (parseTypeDeclaration(builder))
                    ...
                    XQueryPluginBundle.message("parser.error.expected-variable-assign-or-keyword", "as")
            } else {
    ...
    private fun parseSwitchExpr(builder: PsiBuilder): Boolean {
        ...
        if (marker != null) {
            ...
            parseWhiteSpaceAndCommentTokens(builder)
            if (!parseExpr(builder, XQueryElementType2.EXPR)) {
                builder.error(XPathBundle.message("parser.error.expected-expression"))
                haveErrors = true
            }
        ...
        if (marker != null) {
            ...
            if (parseCatchErrorList(builder)) {
                ...
                if (type == CatchClauseType.MARK_LOGIC) {
                    builder.error(XQueryPluginBundle.message("parser.error.multiple-marklogic-catch-clause"))
                }
                builder.advanceLexer()
    ...
    private fun parseUpdateExpr(builder: PsiBuilder, type: IElementType?): Boolean {
        ...
        if (parseComparisonExpr(builder, type)) {
            ...
            while (builder.matchTokenType(XQueryTokenType.K_UPDATE)) {
                ...
                } else if (!parseExpr(builder, XQueryElementType2.EXPR)) {
                    builder.error(XPathBundle.message("parser.error.expected-expression"))
                }
                parseWhiteSpaceAndCommentTokens(builder)
            }
    ...
    override fun parseComparisonExpr(builder: PsiBuilder, type: IElementType?): Boolean {
        ...
        if (parseFTContainsExpr(builder, type)) {
            ...
            builder.errorOnTokenType(
                XPathTokenType.LESS_THAN, XQueryPluginBundle.message("parser.error.comparison-no-lhs-or-direlem")
            ) || builder.errorOnTokenType(
    ...
    override fun parseInlineFunctionExpr(builder: PsiBuilder): Boolean {
        ...
        if (builder.matchTokenType(XPathTokenType.K_FUNCTION)) {
            ...
            if (!parseEnclosedExprOrBlock(builder, XQueryElementType2.FUNCTION_BODY, BlockOpen.REQUIRED, BlockExpr.OPTIONAL) && !haveErrors) {
                builder.error(XPathBundle.message("parser.error.expected", "{"))
                parseExpr(builder, XQueryElementType2.EXPR)
    ...
    private fun parseStringConstructor(builder: PsiBuilder): Boolean {
        ...
        if (marker != null) {
            ...
            if (!builder.matchTokenType(XQueryTokenType.STRING_CONSTRUCTOR_END)) {
                builder.error(XQueryPluginBundle.message("parser.error.incomplete-string-constructor"))
            }
    ...
    private fun parseDirElemConstructor(builder: PsiBuilder, depth: Int): Boolean {
        ...
        if (builder.matchTokenType(XQueryTokenType.OPEN_XML_TAG)) {
            builder.errorOnTokenType(XQueryTokenType.XML_WHITE_SPACE, XQueryPluginBundle.message("parser.error.unexpected-whitespace"))
            parseQNameOrWildcard(builder, XQueryElementType2.QNAME, false)
            ...
            if (builder.matchTokenType(XQueryTokenType.SELF_CLOSING_XML_TAG)) {
                ...
                if (builder.matchTokenType(XQueryTokenType.CLOSE_XML_TAG)) {
                    ...
                    builder.error(XQueryPluginBundle.message("parser.error.expected-closing-tag"))
                }
            } else {
    ...
    private fun parseDirCommentConstructor(builder: PsiBuilder): Boolean {
        ...
        if (marker != null) {
            ...
            if (builder.matchTokenType(XQueryTokenType.XML_COMMENT_END_TAG)) {
                ...
                marker.done(XQueryElementType.DIR_COMMENT_CONSTRUCTOR)
                builder.error(XQueryPluginBundle.message("parser.error.incomplete-xml-comment"))
            }
            return true
        }
    ...
    private fun parseDirPIConstructor(builder: PsiBuilder): Boolean {
        ...
        if (marker != null) {
            ...
            if (builder.matchTokenType(XPathTokenType.WHITE_SPACE)) {
                builder.error(XQueryPluginBundle.message("parser.error.unexpected-whitespace"))
                haveErrors = true
            }
            ...
            if (!builder.matchTokenType(XQueryTokenType.PROCESSING_INSTRUCTION_CONTENTS) && !haveErrors) {
                builder.error(XQueryPluginBundle.message("parser.error.expected-pi-contents"))
                haveErrors = true
            }
    ...
    private fun parseDirElemContent(builder: PsiBuilder, depth: Int): Boolean {
        ...
        while (true) {
            if (
                ...
                builder.errorOnTokenType(XPathTokenType.BLOCK_CLOSE, XQueryPluginBundle.message("parser.error.mismatched-exclosed-expr")) ||
                builder.errorOnTokenType(XQueryTokenType.EMPTY_ENTITY_REFERENCE, XQueryPluginBundle.message("parser.error.empty-entity"))
            ) {
                matched = true
    ...
    private fun parseCDataSection(builder: PsiBuilder, context: IElementType?): Boolean {
        ...
        if (builder.matchTokenType(XQueryTokenType.CDATA_SECTION_START_TAG)) {
            if (context == null) {
                errorMarker.error(XQueryPluginBundle.message("parser.error.cdata-section-not-in-element-content"))
            } else {
                errorMarker.drop()
            }
            ...
            if (builder.matchTokenType(XQueryTokenType.CDATA_SECTION_END_TAG)) {
                ...
                builder.advanceLexer() // XQueryTokenType.UNEXPECTED_END_OF_BLOCK
                marker.done(XQueryElementType.CDATA_SECTION)
                builder.error(XQueryPluginBundle.message("parser.error.incomplete-cdata-section"))
            }
            return true
        }
        ...
        errorMarker.drop()
        marker.drop()
        return builder.errorOnTokenType(XQueryTokenType.CDATA_SECTION_END_TAG, XQueryPluginBundle.message("parser.error.end-of-cdata-section-without-start"))
    }
    ...
    private fun parseCompElemConstructor(builder: PsiBuilder): Boolean {
        ...
        if (marker != null) {
            ...
            if (
                !parseEQNameOrWildcard(builder, XQueryElementType2.QNAME, false) &&
                !parseEnclosedExprOrBlock(builder, null, BlockOpen.REQUIRED, BlockExpr.REQUIRED)
            ) {
                if (builder.tokenType === XPathTokenType.STRING_LITERAL_START) {
                    val errorMarker = builder.mark()
                    parseStringLiteral(builder)
                    errorMarker.error(XQueryPluginBundle.message("parser.error.expected-qname-or-braced-expression"))
                } else {
                    marker.rollbackTo()
                    return false
                }
            ...
            parseWhiteSpaceAndCommentTokens(builder)
            parseEnclosedExprOrBlock(builder, XQueryElementType2.ENCLOSED_CONTENT_EXPR, BlockOpen.REQUIRED, BlockExpr.OPTIONAL)
    ...
    private fun parseCompNamespaceConstructor(builder: PsiBuilder): Boolean {
        ...
        if (marker != null) {
            ...
            if (
                ...
                !parseEnclosedExprOrBlock(builder, XQueryElementType2.ENCLOSED_PREFIX_EXPR, BlockOpen.REQUIRED, BlockExpr.OPTIONAL)
            ) {
                if (builder.tokenType === XPathTokenType.STRING_LITERAL_START) {
                    val errorMarker = builder.mark()
                    parseStringLiteral(builder)
                    errorMarker.error(XQueryPluginBundle.message("parser.error.expected-identifier-or-braced-expression"))
                } else {
                    marker.rollbackTo()
                    return false
                }
            ...
            parseWhiteSpaceAndCommentTokens(builder)
            parseEnclosedExprOrBlock(builder, XQueryElementType2.ENCLOSED_URI_EXPR, BlockOpen.REQUIRED, BlockExpr.OPTIONAL)
    ...
    private fun parseCompPIConstructor(builder: PsiBuilder): Boolean {
        ...
        if (marker != null) {
            ...
            if (
                !parseQNameOrWildcard(builder, XQueryElementType2.NCNAME, false) &&
                !parseEnclosedExprOrBlock(builder, null, BlockOpen.REQUIRED, BlockExpr.REQUIRED)
            ) {
                if (builder.tokenType === XPathTokenType.STRING_LITERAL_START) {
                    val errorMarker = builder.mark()
    ...
    private fun parseFTWeight(builder: PsiBuilder): Boolean {
        ...
        if (marker != null) {
            ...
            parseWhiteSpaceAndCommentTokens(builder)
            if (!parseExpr(builder, XQueryElementType2.EXPR) && !haveError) {
                builder.error(XPathBundle.message("parser.error.expected-expression"))
                haveError = true
            }
    ...
    private fun parseFTMatchOption(builder: PsiBuilder): Boolean {
        ...
        if (
            ...
            builder.error(XQueryPluginBundle.message("parser.error.expected-keyword-or-token", "FTMatchOption", "fuzzy"))
            marker.drop()
            return false
        }
        return true
        ...
        if (builder.matchTokenType(XQueryTokenType.K_THESAURUS)) {
            ...
            if (!builder.matchTokenType(XQueryTokenType.K_DEFAULT) && !parseFTThesaurusID(builder)) {
                if (hasParenthesis) {
                    ...
                    builder.error(XQueryPluginBundle.message("parser.error.expected-keyword-or-token", "(", "at, default"))
                }
                haveError = true
            }
            ...
            if (hasParenthesis) {
                ...
                haveComma = builder.errorOnTokenType(XPathTokenType.COMMA, XQueryPluginBundle.message("parser.error.full-text.multientry-thesaurus-requires-parenthesis"))
            ...
            if (hasParenthesis) {
                if (!builder.matchTokenType(XPathTokenType.PARENTHESIS_CLOSE) && !haveError) {
                    builder.error(XQueryPluginBundle.message("parser.error.expected-either", ",", ")"))
                }
            } else if (!haveError) {
                builder.errorOnTokenType(XPathTokenType.PARENTHESIS_CLOSE, XQueryPluginBundle.message("parser.error.expected-keyword-or-token", ";", "using"))
            } else {
    ...
    private fun parseFTStopWordsInclExcl(builder: PsiBuilder): Boolean {
        ...
        if (marker != null) {
            ...
            if (!parseFTStopWords(builder)) {
                builder.error(XQueryPluginBundle.message("parser.error.expected-keyword-or-token", "(", "at"))
            }
            ...
            while (haveNext) {
                ...
                if (isExtensible) {
                    ...
                    if (!builder.matchTokenType(XPathTokenType.COMMA)) {
                        ...
                        extMarker.error(XQueryPluginBundle.message("parser.error.tuple-wildcard-with-names-after"))
                    }
                ...
                if (builder.matchTokenType(XPathTokenType.STAR)) {
                    ...
                    builder.error(XQueryPluginBundle.message("parser.error.expected-either", "NCName", "*"))
                    haveError = true
                }
            }
    ...
    private fun parseUnionType(builder: PsiBuilder): Boolean {
        ...
        if (marker != null) {
            ...
            while (builder.matchTokenType(XPathTokenType.COMMA)) {
                parseWhiteSpaceAndCommentTokens(builder)
                if (!parseEQNameOrWildcard(builder, XQueryElementType2.QNAME, false) && !haveError) {
                    builder.error(XPathBundle.message("parser.error.expected", "QName"))
                    haveError = true
                }
                parseWhiteSpaceAndCommentTokens(builder)
    ...
    override fun parseMapTest(builder: PsiBuilder): Boolean {
        ...
        if (marker != null) {
            ...
            if (builder.matchTokenType(XPathTokenType.STAR)) {
                ...
                builder.error(XQueryPluginBundle.message("parser.error.expected-either", "UnionType", "AtomicOrUnionType"))
                haveError = true
    ...
    private fun parseArrayNodeTest(builder: PsiBuilder, isAnyOnly: Boolean = false): ParseStatus {
        ...
        if (marker != null) {
            ...
            } else if (builder.errorOnTokenType(XPathTokenType.STAR, XQueryPluginBundle.message("parser.error.expected-either", "StringLiteral", ")"))) {
                type = XQueryElementType.ANY_ARRAY_NODE_TEST
                status = ParseStatus.MATCHED_WITH_ERRORS
            } else {
                type = XQueryElementType.ANY_ARRAY_NODE_TEST
    ...
    private fun parseBooleanNodeTest(builder: PsiBuilder): ParseStatus {
        ...
        if (marker != null) {
            ...
            if (parseStringLiteral(builder)) {
                ...
                builder.errorOnTokenType(XPathTokenType.STAR, XQueryPluginBundle.message("parser.error.expected-either", "StringLiteral", ")"))
        ...
        while (stringMarker != null) {
            if (
                ...
                builder.errorOnTokenType(
                    ...
                    XQueryPluginBundle.message("parser.error.empty-entity")
                ) ||
                builder.matchTokenType(XPathTokenType.BAD_CHARACTER)
            ) {
                //
        ...
        while (true) {
            if (
                ...
                builder.errorOnTokenType(
                    ...
                    XQueryPluginBundle.message("parser.error.misplaced-entity")
                )
            ) {
                skipped = true
            } else {
    ...
    override val URI_QUALIFIED_NAME: IElementType = XQueryElementType2.URI_QUALIFIED_NAME
    ...
    override val NCNAME: IElementType = XQueryElementType2.NCNAME
    override val QNAME: IElementType = XQueryElementType2.QNAME
    ...
    override fun parseQNameSeparator(builder: PsiBuilder, type: IElementType?): Boolean {
        if (
            ...
            if (type === XQueryElementType2.NCNAME || type === XQueryElementType2.PREFIX) {
                val errorMarker = builder.mark()
                builder.advanceLexer()
