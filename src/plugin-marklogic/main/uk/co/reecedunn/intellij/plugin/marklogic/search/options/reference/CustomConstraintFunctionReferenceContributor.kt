...
class CustomConstraintFunctionReferenceContributor : PsiReferenceContributor(), ElementPattern<PsiElement> {
    ...
    override fun accepts(o: Any?): Boolean {
        ...
        return when (parent.localName) {
            "parse" -> node.localName == "apply"
            "start-facet" -> node.localName == "apply"
            "finish-facet" -> node.localName == "apply"
            else -> false
        }
    }
