...
declare %a:since("basex", "8.5")
        %a:until("basex", "9.2", "util:item#2") function util:item-at($sequence as item()*, $position as xs:double) as item()? external;
...
declare %a:since("basex", "8.5")
        %a:until("basex", "9.2", "util:range#3") function util:item-range($sequence as item()*, $first as xs:double, $last as xs:double) as item()* external;

declare %a:since("basex", "9.2") function util:range($sequence as item()*, $first as xs:double, $last as xs:double) as item()* external;

declare %a:since("basex", "8.5")
        %a:until("basex", "9.2", "util:last#1") function util:last-from($sequence as item()*) as item()? external;
...
declare %a:since("basex", "9.5")
        %a:until("basex", "9.7", "util:count-within#1") function util:within($sequence as item()*, $min as xs:integer) as xs:boolean external;

declare %a:since("basex", "9.5")
        %a:until("basex", "9.7", "util:count-within#2") function util:within($sequence as item()*, $min as xs:integer, $max as xs:integer) as xs:boolean external;
